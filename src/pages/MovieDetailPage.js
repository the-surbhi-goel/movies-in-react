...
import { ImDbPath, PosterPath } from "../constants/APIsCons";
import movieImage from "../assets/movieImage.png";
import useFetch from "../hooks/useFetch";
import { useTitle } from "../hooks/useTitle";

const MovieDetailPage = ({title}) => {
  const params = useParams();
  const { data: movie } = useFetch("movie/" + params.id);
  useTitle("Result for " + movie.title);
  ...
  return (
    <section className="flex justify-around flex-wrap py-5">
      <div className="max-w-sm">
        <img
          className="rounded"
          src={movie?.poster_path ? `${PosterPath}${movie?.poster_path}` : movieImage}
          alt="imag"
        />
      </div>
      <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
        ...
        <p className="my-4text-center lg:text-left">
        ...
        <p className="my-4 text-center lg:text-left">
          ...
          <a href={`${ImDbPath}${movie?.imdb_id}`} target="_blank" rel="noreferrer">
            {movie?.imdb_id}
          </a>
        </p>
      </div>
